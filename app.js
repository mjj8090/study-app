...
function renderStudy() {
  ...
  return `<div class="page">
    ...
    ${studyMode === 'free' ? `<div class="btn-group mt-8">
...
function renderFreeSetup() {
  ...
  return `
    ...
    <div class="section-label">题号范围（可选）</div>
    <div class="plan-row">
      <span>从第</span>
      <input type="number" id="freeStartInput" placeholder="1" min="1" max="${maxId}" inputmode="numeric">
      <span>题到第</span>
      <input type="number" id="freeEndInput" placeholder="${maxId}" min="1" max="${maxId}" inputmode="numeric">
      <span>题</span>
    </div>
    ...
    <div class="px-16 mt-12">
      <button class="btn btn-primary full" id="startFreeStudyBtn">开始自由背</button>
    </div>`;
}
...
function attachEvents() {
  ...
  on('startFreeStudyBtn', () => {
    ...
    if (startInp && startInp.value) freeStartId = startInp.value;
    if (endInp && endInp.value) freeEndId = endInp.value;
